...
<header class="top-0 z-50 mx-auto max-w-7xl md:sticky md:top-4 relative">
	<nav>
		<div
			class="flex flex-col justify-between rounded-b-lg bg-white px-4 py-2 md:m4 md:flex-row md:items-center md:rounded-xl"
		>
			<div class="flex items-center justify-between">
				<a
					href="/"
					aria-label="Homepage"
					class="text-xl font-extrabold tracking-tighter text-blue-900">Inicio</a
				>
				<button
					aria-expanded={open}
					aria-label="Open Menu"
					class="block p-2 text-2xl text-blue-700 md:hidden"
					on:click={() => (open = true)}
				>
					<IconMenu />
				</button>
			...
			>
				<li>
					<button
						...
						class="fixed right-4 top-3 block p-2 text-2xl text-blue-700 md:hidden"
						on:click={() => (open = false)}
					>
						<IconClose />
					</button>
				</li>
				{#each settings.data.nav_item as { text, link }}
					<li class="first:mt-8">
						<NavbarLink field={link} {text} {onLinkClick} type="mobile" />
					</li>
				{/each}
			...
			<ul class="relative z-50 hidden flex-row items-center gap-1 bg-transparent py-0 md:flex">
				{#each settings.data.nav_item as { text, link }}
					<li>
						<NavbarLink field={link} {text} {onLinkClick} type="desktop" />
					</li>
				{/each}
				{#if isFilled.link(settings.data.cta_link)}
